  ...
  return (
    <Card className="card-glow overflow-hidden group animate-slide-up">
      ...
      <CardHeader className="pb-2 md:pb-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <Badge 
            ...
            className={`text-xs ${isAvailable ? "bg-green-500/20 text-green-400 border-green-500/30" : ""}`}
          >
      ...
      <CardContent className="space-y-3 md:space-y-4">
        <p className="text-sm text-muted-foreground line-clamp-2 min-h-[2.5rem] md:min-h-[40px]">
          {item.desc}
        </p>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs md:text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="font-mono text-xs">{shortenAddress(item.seller)}</span>
          </div>
        </div>
      </CardContent>
